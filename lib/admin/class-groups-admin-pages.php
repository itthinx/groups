...
/**
 * Pages admin integration with Groups.
 */
class Groups_Admin_Pages {

	const GROUPS = 'groups_page_groups';
	
	/**
	 * Hooks into filters to add the capabilities column to the pages table.
	 */
	...
	public static function setup() {
		if ( current_user_can( GROUPS_ACCESS_GROUPS ) ) {
			...
			add_filter('manage_pages_columns', array( __CLASS__, 'manage_pages_columns' ) );
			// args: string $column_name, int $page_id
			add_action( 'manage_pages_custom_column', array( __CLASS__, 'manage_pages_custom_column' ), 10, 2 );
		}
	...
	/**
	 * Adds a new column to the pages table to show the capabilities
	 * 
	 * @param array $column_headers
	 * @return array column headers
	 */
	public static function manage_pages_columns( $column_headers ) {
	 ...
	 */
	public static function manage_pages_custom_column( $column_name, $page_id ) {
		$output = "";
		switch ( $column_name ) {
			case self::GROUPS :
				$user = new Groups_User( get_current_user_id() );
			
				$read_caps = get_post_meta( $page_id, Groups_Post_Access::POSTMETA_PREFIX . Groups_Post_Access::READ_POST_CAPABILITY );
